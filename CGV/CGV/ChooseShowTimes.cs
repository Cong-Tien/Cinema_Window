...
namespace CGV
{
    public partial class ChooseShowTimes : Form
    {
        ...
        private void dgvShowTime_DoubleClick(object sender, EventArgs e)
        {
            ...
            Showtime i = Context.Showtimes.FirstOrDefault(p => p.IDShowtimes == cmbShowTime.Text);
              if(i!=null)
            {
                if(i.Statues==0)
                {
                    MessageBox.Show("Lịch chiếu này chưa được tạo \n Vui lòng chọn lịch chiếu khác hoặc đi tạo vé","CGV xin thông báo");
                }
                else
                {
